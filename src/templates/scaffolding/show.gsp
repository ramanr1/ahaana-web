        ...
        <ol class="property-list ${domainClass.propertyName}">
            ...
            <g:if test="\${${propertyName}?.${p.name}}">
                <li class="fieldcontain">
                    ...
                    <% if (p.isEnum()) { %>
                    <span class="uneditable-input" aria-labelledby="${p.name}-label"><g:fieldValue
                            bean="\${${propertyName}}" field="${p.name}"/></span>
                    <% } else if (p.oneToMany || p.manyToMany) { %>
                    <g:each in="\${${propertyName}.${p.name}}" var="${p.name[0]}">
                        <span class="uneditable-input" aria-labelledby="${p.name}-label"><g:link
                    ...
                    <span class="uneditable-input" aria-labelledby="${p.name}-label"><g:formatBoolean
                            boolean="\${${propertyName}?.${p.name}}"/></span>
                    <% } else if (p.type == Date || p.type == java.sql.Date || p.type == java.sql.Time || p.type == Calendar) { %>
                    <span class="uneditable-input" aria-labelledby="${p.name}-label"><g:formatDate
